...
def do_punet_source_predictions(device, data_path:str, pred_path:str):
    ...
    for cellname in cell_list:
        ...
        for cellname_ in cell_list:
            ...
            punet_pseudo_prediction(input_image_path=input_path, output_pred_path=output_path, model=model, 
                                 device=device, prior_samples=16, cellname_=cellname_, split_name=split_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--get_pseudo_labels", action='store_true', help="Generates the Pseudo Labels and Consensus Masks, subjected to the already trained source domains")
    parser.add_argument("--train", action='store_true', help="Enables target-transfer training on LiveCELL dataset")
    parser.add_argument("--predict", action='store_true', help="Obtains target-transfer predictions on LiveCELL test-set")
