...
def get_target_livecell_loaders(
    path: str, ctype: list, patch_shape=(256, 256), my_augs=my_weak_augmentations()
):
...
def do_adamt_training(args, device, data_path: str):
    ...
    for trg in cell_types:
        for src in cell_types:
            if src != trg:
                print(f"Transferring {src} network learnings on {trg} using AdaMT")
                ...
                if args.consensus is True and args.masking is False:
                    my_name = f"adamt-livecell-source-{src}-target-{trg}-consensus-weighting"
                elif args.masking is True and args.masking is True:
                    my_name = f"adamt-livecell-source-{src}-target-{trg}-consensus-masking"
                else:
                    my_name = f"adamt-livecell-source-{src}-target-{trg}"
...
def main(args):
    ...
    if args.train:
        print("Training PUNet on AdaMT framework on LiveCELL dataset")
        do_adamt_training(args, data_path=args.data, device=device)

    if args.predict:
        print("Getting predictions on LiveCELL dataset from the trained AdaMT framework")
        do_adamt_predictions(args, data_path=args.data, pred_path=args.pred_path, device=device)

    if args.evaluate:
        print("Evaluating the AdaMT predictions of LiveCELL dataset")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", action='store_true', help="Enables AdaMT-based training on LiveCELL dataset")
    parser.add_argument("--predict", action='store_true', help="Obtains AdaMT predictions on LiveCELL test-set")
    ...
    parser.add_argument(
        "--pred_path", type=str, default="~/predictions/livecell/",
