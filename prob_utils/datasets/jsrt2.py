...
def get_jsrt_s2_loader(
    ...
    if split == "val":
        ...
        label_paths = glob(os.path.join(data_path, "label_train", "*.png"))
    else:
        raw_paths = glob(os.path.join(data_path, f"org_{split}", "*.bmp"))
        label_paths = glob(os.path.join(data_path, f"label_{split}", "*.png"))
    ...
    assert len(raw_paths) > 0, raw_paths
