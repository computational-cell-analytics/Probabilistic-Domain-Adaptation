...
class AbstractProb3DUNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if flow_type == 'radial':
            self.flow_type = utils.Radial
        elif flow_type == 'planar':
            self.flow_type = utils.Planar
        else:
            self.flow_type = None
        ...
        if isinstance(f_maps, int):
            f_maps = utils.number_of_features_per_level(f_maps, num_levels=num_levels)

        assert isinstance(f_maps, list) or isinstance(f_maps, tuple)
        assert len(f_maps) > 1, "Required at least 2 levels in the U-Net"

        if isinstance(encoders_f_maps, int):
            encoders_f_maps = utils.number_of_features_per_level(encoders_f_maps, num_levels=encoder_num_levels)
        ...
        # create encoder path
        self.encoders = utils.create_encoders(
            in_channels,
            f_maps,
            basic_module,
            conv_kernel_size,
        ...
        # create decoder path
        self.decoders = utils.create_decoders(
            f_maps,
            basic_module,
            conv_kernel_size,
            conv_padding,
        ...
        if self.flow_type is not None:
            ...
            self.posterior = utils.PosteriorNetWithNormalizingFlow(
                in_channels + out_channels,
                encoders_f_maps,
                basic_module,
                conv_kernel_size,
                conv_padding,
                posterior_layer_order,
                encoder_num_groups,
                pool_kernel_size,
                self.latent_dim,
            ...
            self.posterior = utils.PosteriorNet(
                in_channels + out_channels,
                encoders_f_maps,
                basic_module,
                conv_kernel_size,
    ...
    def forward(self, input, target=None):
        ...
        if target is not None:  # train or validation, grab posterior
            if self.flow_type is not None:
    ...
    def kl_divergence(self, posterior_latent_space, prior_latent_space, log_det_j_q=None, z_q=None, z0_q=None):
        ...
        if log_det_j_q is None and z_q is None and z0_q is None:
