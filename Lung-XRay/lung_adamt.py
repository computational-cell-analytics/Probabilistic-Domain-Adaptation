    ...
    for source_domain in lung_list:
        for target_domain in lung_list:
            if source_domain != target_domain:
                ...
                source_train_loader = get_source_lung_loaders(lung_domain_name=source_domain,
                                                              root_input_dir=data_path)

                target_train_loader, target_val_loader = get_target_lung_loaders(lung_domain_name=target_domain,
                                                                                 root_input_dir=data_path)
    ...
    for source_lung_domain in lung_domain_names:
        for target_lung_domain in lung_domain_names:
            if source_lung_domain != target_lung_domain:
                ...
                if args.consensus is True and args.masking is False:
                    model_save_dir = f"adamt-lung-source-{source_lung_domain}-target-{target_lung_domain}-consensus-weighting/best.pt"

                elif args.masking is True and args.masking is True:
                    model_save_dir = f"adamt-lung-source-{source_lung_domain}-target-{target_lung_domain}-consensus-masking/best.pt"
