...
from prob_utils.my_datasets import get_jsrt_s1_loader, get_jsrt_s2_loader, get_nih_loader, get_montgomery_loader
...
def get_dual_lung_loaders(lung_domain_name:str, root_input_dir:str, my_augs=my_weak_augmentations()):
...
def do_mean_teacher_training(args, device, data_path:str, source_ckpt_path:str):
    ...
    lung_xray_types = ["jsrt1", "jsrt2", "nih", "montgomery"]

    for trg in lung_xray_types:
        for src in lung_xray_types:
            if src!=trg:

                print(f"Transferring {src} network learnings on {trg} using Mean-Teacher")

                my_ckpt = source_ckpt_path + f"punet-source-lung-{src}/best.pt"
                if os.path.exists(my_ckpt) is False:
                    print("The checkpoint directory couldn't be found/source network hasn't been trained")
                    continue

                train_loader, val_loader = get_dual_lung_loaders(lung_domain_name=trg,
                ...
                if args.consensus is True and args.masking is False:
                    my_name = f"mean-teacher-lung-source-{src}-target-{trg}-consensus-weighting"

                elif args.masking is True and args.masking is True:
                    my_name = f"mean-teacher-lung-source-{src}-target-{trg}-consensus-masking"

                else:
                    my_name = f"mean-teacher-lung-source-{src}-target-{trg}"
    ...
    for source_lung_domain in lung_domain_names:
        for target_lung_domain in lung_domain_names:
            if source_lung_domain != target_lung_domain:
                ...
                if args.consensus is True and args.masking is False:
                    model_save_dir = f"mean-teacher-lung-source-{source_lung_domain}-target-{target_lung_domain}-consensus-weighting/best.pt"

                elif args.masking is True and args.masking is True:
                    model_save_dir = f"mean-teacher-lung-source-{source_lung_domain}-target-{target_lung_domain}-consensus-masking/best.pt"

                else:
                    model_save_dir = f"mean-teacher-lung-source-{source_lung_domain}-target-{target_lung_domain}/best.pt"
                ...
                output_path = pred_path + f"mean_teacher/source-{source_lung_domain}-target-{target_lung_domain}/"
    ...
    for source_lung_domain in lung_domain_names:
        for target_lung_domain in lung_domain_names:
            if source_lung_domain != target_lung_domain:
                ...
                punet_pred_path = pred_path + f"mean_teacher/source-{source_lung_domain}-target-{target_lung_domain}/"
...
def main(args):
    ...
    if args.train:
        print("Training PUNet on Mean-Teacher framework on Lung X-Ray datasets")
        do_mean_teacher_training(args, data_path=args.data, source_ckpt_path=args.source_checkpoints, device=device)

    if args.predict:
        print("Getting predictions on Lung X-Ray datasets from the trained Mean-Teacher framework")
        do_mean_teacher_predictions(args, data_path=args.data, pred_path=args.pred_path, device=device)

    if args.evaluate:
        print("Evaluating the Mean-Teacher predictions of Lung X-Ray dataset")
        do_mean_teacher_evaluations(data_path=args.data, pred_path=args.pred_path)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()

    parser.add_argument("--train", action='store_true', help="Enables Mean-Teacher training on Lung X-Ray datasets")
    parser.add_argument("--predict", action='store_true', help="Obtains Mean-Teacher predictions on Lung X-Ray test-set")
    ...
    parser.add_argument("--pred_path", type=str, default="~/predictions/lung-xray/", help="Path where predictions will be saved")
